...
namespace NeoErp.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            Session.Clear();
            var jsonPath = Server.MapPath("~/App_Data/Json/DbInfoSettings.json");
            if (!System.IO.File.Exists(jsonPath))
            {
                ViewBag.DatabaseList = new List<SelectListItem>();
                ModelState.AddModelError("", "Database configuration file not found.");
                return View();
            }
            var json = System.IO.File.ReadAllText(jsonPath);
            var config = JsonConvert.DeserializeObject<DatabaseConfig>(json);
            if (config?.databases == null || !config.databases.Any())
            {
                ViewBag.DatabaseList = new List<SelectListItem>();
                ModelState.AddModelError("", "No database configuration found.");
                return View();
            }
            ViewBag.DatabaseList = config.databases
                .Where(db => db.active?.Equals("Y", StringComparison.OrdinalIgnoreCase) == true)
        ...
        public ActionResult LogIn(User model)
        {
            try
            {
                ViewBag.DatabaseList = GetDatabasesFromJson()

                    .Select(db => new SelectListItem
                    {
                        Text = db.ContainsKey("Name") ? db["Name"] : "Unknown",
                        Value = db.ContainsKey("Name") ? db["Name"] : "Unknown",
                        Selected = db.ContainsKey("Name") && db["Name"] == model.SelectedDatabase
                    })
                    .ToList();
                ViewBag.error = false;
                ...
                {
                    ViewBag.error = true;
                    ViewBag.Message = "Please select a database.";
                    return View(model);
                }

                Session["SelectedDbKey"] = model.SelectedDatabase;
                ConnectionManager.SetSelectedDatabase(model.SelectedDatabase);
                ...
                var result = _authenticationService.SignIn(model.UserName, model.Password, model.company_name, model.branch_code, model.SelectedDatabase).ToList();
                _logErp.InfoInFile($@"User authenticated for username : {model.UserName}  password: {model.Password} company:{model.company_name} selectedDatabase: {model.SelectedDatabase} and branch: {model.branch_name}");
                if (result.Count > 0)
                {
                    ...
                    //ViewBag.CompanyCode = userData.company_code;
                    //ViewBag.BranchCode = userData.branch_code;
                    //ViewBag.UserId = userData.User_id.ToString();
        ...
        public List<Dictionary<string, string>> GetDatabasesFromJson()
        {
            return ConnectionManager.GetDatabasesFromJson();
        }


        [HttpPost]
        public ActionResult SetDatabase(string SelectedDatabase)
        {
            //Session["SelectedDatabase"] = SelectedDatabase;
            ConnectionManager.SetSelectedDatabase(SelectedDatabase);
            return RedirectToAction("LogIn", "Home");
        }

        [HttpPost]
        public JsonResult SaveDatabaseConfig(DatabaseInfo newDatabase)
        {
            try
            {
                var jsonPath = Server.MapPath("~/App_Data/Json/DbInfoSettings.json");
                ...
                {
                    config = new DatabaseConfig { databases = new List<DatabaseInfo>() };
                }

                var existingDb = config.databases.FirstOrDefault(db =>
                    db.name.Equals(newDatabase.name, StringComparison.OrdinalIgnoreCase));

                if (existingDb != null)
                {
                    return Json(new { success = false, message = "A database with this name already exists." });
                }

                var encryptedPassword = EncryptPassword(newDatabase.dbPassword);
                newDatabase.dbPassword = encryptedPassword;

                config.databases.Add(newDatabase);

                var updatedJson = JsonConvert.SerializeObject(config, Newtonsoft.Json.Formatting.Indented);
                System.IO.File.WriteAllText(jsonPath, updatedJson);

                _logErp.InfoInFile($"New database configuration added: {newDatabase.name}");

                return Json(new { success = true, message = "Database configuration saved successfully." });
            }
            catch (Exception ex)
            {
                _logErp.ErrorInDB($"Error saving database config: {ex.Message}");
        ...
        public JsonResult GetDatabaseList()
        {
            try
            {
                ...
                var databaseList = config.databases
                    .Where(db => db.active?.Equals("Y", StringComparison.OrdinalIgnoreCase) == true)
                    .Select(db => new SelectListItem
                    {
                        Text = db.name,
            ...
            {
                _logErp.ErrorInDB($"Error getting database list: {ex.Message}");
                return Json(new List<SelectListItem>(), JsonRequestBehavior.AllowGet);
            }
        }
        ...
        private string EncryptPassword(string password)
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    using (var command = new OracleCommand($"select fn_encrypt_password('{password}') from dual", connection))
                    {
                        var result = command.ExecuteScalar();
                        encryptedPassword = result?.ToString() ?? string.Empty;
                    }
                }
            }
            catch (Exception ex)
            {
                _logErp.ErrorInDB($"Failed to encrypt password: {ex.Message}");
                throw new Exception($"Failed to encrypt password: {ex.Message}", ex);
            }
        ...
        public ActionResult LogInMobile(User model)
        {
            ...
            var result = _authenticationService.SignIn(model.UserName, model.Password, model.company_name, model.branch_code, model.SelectedDatabase=null).ToList();
        ...
        [AcceptVerbs(HttpVerbs.Get)]
        public ActionResult CompanyBranchList(string UserName, string Password, string SelectedDatabase)
        {
            IEnumerable<CompanyBranchModel> data = _authenticationService.GetAllCompanyBranch(UserName, Password, SelectedDatabase);
            return Json(data, JsonRequestBehavior.AllowGet);
        }
        //public ActionResult CompanyBranchList(string UserName, string Password, Selectedda)
        //{
        //    IEnumerable<CompanyBranchModel> data = _authenticationService.GetAllCompanyBranch(UserName, Password, SelectedDatabase);
        //    return Json(data, JsonRequestBehavior.AllowGet);
        //}
        [AcceptVerbs(HttpVerbs.Get)]
        public ActionResult ListCompanyBranch(List<string> company_code, string UserName, string SelectedDatabase)
        {
            ...
            IEnumerable<BranchInfo> data = _authenticationService.GetBranchControlList(Company, UserName, SelectedDatabase);
            return Json(data, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult GetReportSetting(string reportName)
        {
            try
            {
                ...
                return Json(reportSettingModel, "application/json");
            }
            catch (Exception e)
            {
        ...
        public JsonResult GetFavroiteMenus(string moduleCode = "01")
        {
            try
            {
                ...
                string jsonData = Newtonsoft.Json.JsonConvert.SerializeObject(result.Reverse().ToList(), Newtonsoft.Json.Formatting.Indented).Replace('@', 'R');
        ...
        public ActionResult HelpManual()
        {
            // Path to your PDF file within the project directory
            //string pdfFilePath = Server.MapPath("~/Path/To/Your/PDF.pdf");
            string pdfFilePath = Server.MapPath("~/PDF/Synergy User Manual Book..pdf");
            ...
            using (FileStream fs = new FileStream(pdfFilePath, FileMode.Open, FileAccess.Read))
            {
                ...
                // Return the PDF bytes as a FileStreamResult
        ...
        //public ActionResult DatabaseDebug()
        //{
        //    return View();
        //}
