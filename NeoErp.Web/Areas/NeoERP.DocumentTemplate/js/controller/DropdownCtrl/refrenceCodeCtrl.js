...
DTModule.controller('refrenceCodeCtrl', function ($scope, $rootScope, $http, $routeParams, $window, $filter, formtemplateservice) {
        ...
        function selectRow() {
            ...
            if (checked) {
                ...
                checkedItems.push({
                    ...
                    "MAIN_ITEM_CODE": dataItem.MAIN_ITEM_CODE,
    ...
    $scope.bindReferenceGrid = function () {
        ...
        if (tableName != 'SA_SALES_BLANKET') {
            $scope.referenceGridOptions = {
                ...
                dataSource: {
                    transport: {
                        ...
                        parameterMap: function (options, type) {
                            var data = $.extend({}, options, { referenceModel: DOCUMENT_REFERENCE });
                            delete data.$inlinecount;
                            delete data.$format;
            ...
            let allReferenceData = []; // full data storage

            function detailInitFunction(e) {
                debugger;

                const parentData = e.data;
                const parentVoucher = parentData.VOUCHER_NO;
                const parentQuantity = parentData.QUANTITY;

                // Assuming allReferenceData is in your scope
                const childRows = allReferenceData.filter(row =>
                    row.IS_PARENT_ROW === 'N' && row.VOUCHER_NO === parentVoucher
                );

                if (childRows.length === 0) {
                    $(e.detailCell).html("<div style='padding:10px;color:red;'>No child records found.</div>");
                    return;
                }

                const childGrid = $("<div/>").appendTo(e.detailCell).kendoGrid({
                    dataSource: {
                        data: childRows
                    },
                    scrollable: false,
                    ...
                    columns: [
                        {
                            title: "",
                            width: 40,
                            template: "<input type='checkbox' class='childCheckbox' data-uid='#= uid #' data-parent-voucher='#= VOUCHER_NO #' data-parent-qty='#= QUANTITY #' />"
                        },
                        { field: "VOUCHER_NO", title: "Voucher No" },
                        { field: "VOUCHER_DATE", title: "Voucher Date", template: "#= kendo.toString(kendo.parseDate(VOUCHER_DATE), 'MM/dd/yyyy') #" },
                        ...
                        { field: "UNIT", title: "Unit" },
                        {
                            ...
                            template: "<input type='text' class='form-control quantity-input' value='#= QUANTITY #' data-uid='#= uid #' style='width: 100%; text-align: right;' oninput='this.value = this.value.replace(/[^0-9.]/g, \"\")' />"
                        },
                        { field: "UNIT_PRICE", title: "Unit Price", attributes: { style: "text-align:right;" } },
                        { field: "SERIAL_NO", title: "Serial No", hidden: true },
                        { field: "MAIN_ITEM_CODE", title: "Main Item Code", hidden: true },
                        { field: "IS_PARENT_ROW", title: "Is Parent Row", hidden: true },
                        { field: "RATE", title: "Rate", hidden: true },
                        { field: "HAS_CHILD", title: "Has Child", hidden: true },
                        {
                            field: "TOTAL_PRICE", title: "Total Price", attributes: { style: "text-align:right;" },
                            template: "<input type='text' class='form-control total-price-input' value='#= TOTAL_PRICE #' data-uid='#= uid #' style='width: 100%; text-align: right;' readonly />"
                        },
                        { field: "FORM_CODE", title: "Form Code" }
                    ],
                    dataBound: function () {
                        debugger;
                        const grid = this;

                        // Scope only inside this grid
                        grid.element.find(".childCheckbox").off("click").on("click", function () {
                            debugger;
                            ...
                            const currentVoucherNo = dataItem.VOUCHER_NO;

                            $(row).toggleClass("k-state-selected", this.checked);



                            const checkbox = row.find(".childCheckbox");
                            ...
                            if (isChecked) {
                                ...
                                // Get the voucher number from the clicked row

                                // Filter all rows with same voucher number
                                const matchingRows = gridData.filter(item => item.VOUCHER_NO === currentVoucherNo);

                                // Loop through each matching row and log its quantity
                                var totalQuantity = 0;

                                var previousQuantity = 0;
                                matchingRows.forEach(item => {

                                    const row = childGrid.table.find(`tr[data-uid='${item.uid}']`);
                                    const checkbox = row.find(".childCheckbox");
                                    var currentRowQuantity = parseInt(item.QUANTITY, 10);
                                    // Use .prop("checked") for accuracy
                                    const isCheckedChildRow = checkbox.prop("checked");

                                    if (isCheckedChildRow) {
                                        totalQuantity = currentRowQuantity + previousQuantity;
                                        previousQuantity = totalQuantity;
                                        if (totalQuantity > parentQuantity) {
                                            displayPopupNotification("Child rows quanity cannot be higher then Parent row", "warning");
                                            row.find(".quantity-input").val(0);
                                            row.find(".total-price-input").val(0);


                                            dataItem.QUANTITY = 0; // ← Silent update, no rerender!
                                            dataItem.TOTAL_PRICE = 0;
                                            return;
                                        }
                                    }
                                ...
                                row.addClass("k-state-selected");
                                $(this).attr('checked', true);
                                checkedIds[dataItem.ORDER_NO] = isChecked;
                                checkedItems.push({
                                    "VOUCHER_NO": dataItem.VOUCHER_NO,
                                    "SERIAL_NO": dataItem.SERIAL_NO,
                                    "TABLE_NAME": $scope.RefTableName,
                                    ...
                                    "IS_PARENT_ROW": dataItem.IS_PARENT_ROW,
                            ...
                            //$(".childCheckbox").on("click", selectRow);

                        });

                        grid.element.find(".quantity-input").off("input").on("input", function () {
                            debugger;
                            const uid = $(this).data("uid");
                            const row = grid.table.find("tr[data-uid='" + uid + "']");

                            $(row).addClass("k-state-selected"); // just mark it selected if edited

                            // Grab the value entered (if you need it)
                            var newQuantity = $(this).val();

                            const dataItem = grid.dataSource.getByUid(uid);

                            const unitPrice = dataItem.UNIT_PRICE;

                            var totalPrice = newQuantity * unitPrice;

                            const serialNo = dataItem.SERIAL_NO;

                            const itemCode = dataItem.ITEM_CODE;

                            const voucherNo = dataItem.VOUCHER_NO;

                            // Update the TOTAL_PRICE input manually
                            row.find(".total-price-input").val(totalPrice.toFixed(2));

                            if (dataItem) {
                                dataItem.QUANTITY = newQuantity; // ← Silent update, no rerender!
                                dataItem.TOTAL_PRICE = totalPrice;
                            }
                            ...
                            if (isChecked) {
                                ...
                                angular.forEach(checkedItems, function (item, index) {
                                    debugger;
                                    if (item.VOUCHER_NO == voucherNo && item.ITEM_CODE == itemCode && item.SERIAL_NO == serialNo) {
                                        checkedItems[index].QUANTITY = newQuantity;
                                        checkedItems[index].TOTAL_PRICE = totalPrice;
                                    }
                                });
            ...
            $scope.referenceGridOptions = {
                ...
                dataSource: {
                    ...
                    schema: {
                        data: function (response) {
                            debugger;
                            // Store all rows
                            allReferenceData = response;

                            // Return only parent rows for the main grid
                            return response.filter(row => row.IS_PARENT_ROW === 'Y');
                        }
                    },
                    group: {
                        //field: "ORDER_NO",
                ...
                dataBound: function (e) {
                    ...
                    //const data = this.dataSource.view(); // This is paged data, use .data() for full
                    //allReferenceData = this.dataSource.data(); // Gets all data in the grid
                    ...
                    $(".quantity-input").on("input", function () {
                        ...
                        dataItem.set("QUANTITY", $(this).val());
                    });
        ...
        $("#referenceGrid").on("input", ".quantity-input", function () {
            debugger;
            ...
            var quantityVal = $(this).val(); // <-- store in variable
            var unitPriceVal = dataItem.UNIT_PRICE;

            var totalPrice = quantityVal * unitPriceVal;

            if (quantityVal > dataItem.CALC_QUANTITY) {
                quantityVal = 0;
                totalPrice = 0;
                displayPopupNotification("Quantity cannot be more then: " + dataItem.CALC_QUANTITY, "warning");
            }

            dataItem.TOTAL_PRICE = totalPrice.toFixed(2);
            dataItem.QUANTITY = quantityVal;

            row.find(".total-price-input").val(totalPrice.toFixed(2));

            row.find(".quantity-input").val(quantityVal);


            if (checkedItems.length > 0) {
                for (var i = 0; i < checkedItems.length; i++) {
                    if (dataItem.VOUCHER_NO === checkedItems[i].VOUCHER_NO && dataItem.ITEM_CODE === checkedItems[i].ITEM_CODE && dataItem.SERIAL_NO === checkedItems[i].SERIAL_NO) {
                        checkedItems[i].QUANTITY = quantityVal;
                        checkedItems[i].TOTAL_PRICE = totalPrice.toFixed(2);
                    }
                }
            }

            //dataItem.set("QUANTITY", $(this).val()); // <-- stored in datasource
        ...
        function selectRow() {
            ...
            // ❌ Prevent selection for parent rows that have child rows
            if (dataItem.IS_PARENT_ROW === 'Y' && dataItem.HAS_CHILD === 'Y') {
                this.checked = false; // Uncheck the box
                $(row).removeClass("k-state-selected"); // Deselect visually
                displayPopupNotification("Child row exists of this Voucher No.", "warning");
                return; // Exit early
            }
    ...
    $scope.bindRefrenceDataToTemplate = function () {
        ...
        //        displayPopupNotification("Quantity and Total Price needs to be more then 0", "warning");
        //        quantityCheck = 1;
        //        return false;
        //    }
        //});
        ...
        //if ($scope.RefTableName == 'SA_SALES_BLANKET') {
        //    angular.forEach(checkedItems, function (item, index) {
        //        debugger;
        //        if (item.QUANTITY <= 0) {
        //            displayPopupNotification("Quantity needs to be more then 0", "warning");
        //            quantityCheck = 1;
        //            return false;
        //        }
        //    });
            ...
            colName = [
                {
                    ...
                    template: function (dataItem) {
                        return "<input type='checkbox' id='${dataItem.VOUCHER_NO}' class='checkbox row-checkbox_" + dataItem.CUSTOMER_CODE + "'>"
                    },
                    width: 50
                },
                //{
