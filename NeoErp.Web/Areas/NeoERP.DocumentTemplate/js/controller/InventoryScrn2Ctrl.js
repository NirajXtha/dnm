...
DTModule.controller('InventoryScrn2Ctrl', function ($scope, $rootScope, $http, $route, $routeParams, $location, inventoryservice, inventoryfactory, $window, $filter, hotkeys, $q) {
    ...
    $scope.controllerName = "InventoryScrn2Ctrl";
    $scope.parentVoucherNo = "";
    ...
    if ($routeParams.orderno != undefined) {
        ...
        $scope.OrderNo = $scope.$parent.invCtrlObject.NextScreenOrderNo;
    ...
    $scope.formcode = $scope.$parent.invCtrlObject.NextScreenFormCode; // 129; // $routeParams.formcode;
    $scope.OrderNo = $scope.$parent.invCtrlObject.NextScreenOrderNo;


    $scope.FormCode = $scope.formcode; // $routeParams.formcode;
    ...
    $scope.receivedFromParentData = {}; // To store data


    $scope.$on('fromParentToChildMasterFieldMapping', function (event, data) {
        console.log("Child received data:", data);
        $scope.receivedFromParentData = data; // store to show in view

        if ($scope.masterModels.hasOwnProperty("FROM_LOCATION_CODE")) {
            $scope.masterModels["FROM_LOCATION_CODE"] = data.RoutingName;
        }

        if ($scope.masterModels.hasOwnProperty("PRODUCTION_QTY")) {
            $scope.masterModels["PRODUCTION_QTY"] = data.Production_Qty;

            if (data.Wt_Per_Ltr && data.Wt_Per_Ltr > 0) {
                $scope.masterModels['PRODUCTION_QTY'] = $scope.masterModels['PRODUCTION_QTY'] / data.Wt_Per_Ltr;
            }
        }

        if ($scope.masterModels.hasOwnProperty("BATCH_NO")) {
            $scope.masterModels["BATCH_NO"] = data.Batch_No;
        }


        if ($scope.masterModels.hasOwnProperty("MANUAL_NO")) {
            $scope.masterModels["MANUAL_NO"] = data.Manual_No;
        }


        if ($scope.masterModels.hasOwnProperty("REFERENCE_NO")) {
            $scope.masterModels["REFERENCE_NO"] = data.Rererence_No;
        }

        $scope.getVoucherDetailForReferenceProduct();
    });

    $scope.isCheckSaveValidationState = false;

    $scope.$on('fromParentToChildValidateCheck', function (event, data) {
        //debugger;
        $scope.isCheckSaveValidationState = data.isCheckValidate;
        $scope.parentVoucherNo = data.voucherNo;
        $scope.SaveDocumentFormData(data.param);
    });
    ...
    $scope.inventoryRefrenceFn = function (response, callback) {
            ...
            if (rows[0].IMAGES_LIST != null || rows[0].IMAGES_LIST != undefined) {
                ...
                if (imageurl.length > 0) {
                    for (var i = 0; i < imageurl.length; i++) {
                        ...
                        if (i == 0) {
                            myInventoryScreen2Dropzone.on("addedfile", function (file) {
                        ...
                        myInventoryScreen2Dropzone.emit("addedfile", mockFile);
                        myInventoryScreen2Dropzone.emit("thumbnail", mockFile, imageurl[i]);
                        myInventoryScreen2Dropzone.emit('complete', mockFile);
                        myInventoryScreen2Dropzone.files.push(mockFile);
        ...
        function setDataOnModal(rows, i) {
            ...
            setTimeout(function () {
                ...
                console.log("called here:" + i);

                $("#productsScrn2_" + i).data('kendoComboBox').dataSource.data([{ ItemCode: rows[i].ITEM_CODE, ItemDescription: rows[i].ITEM_EDESC, Type: "code" }]);
                console.log("value of productCode from kendo===================>>>>" + JSON.stringify($("#productsScrn2_" + i).data('kendoComboBox').dataSource.data()[0].ItemCode));
                $scope.childModels[i].ITEM_CODE = $("#productsScrn2_" + i).data('kendoComboBox').dataSource.data()[0].ItemCode;
            }, 0);
                    ...
                    function (mvalue, mkey) {
                        ...
                        if (mkey === "TO_LOCATION_CODE") {
                            ...
                            $http.get(req).then(function (results) {
                                ...
                                setTimeout(function () {
                                    //   //debugger;
                                    $("#tolocationScrn2").data('kendoComboBox').dataSource.data([{ LocationCode: rows[0].TO_LOCATION_CODE, LocationName: results.data, Type: "code" }]);
                                }, 50);
                            });
                        }
                    });
                    ...
                    if (value['COLUMN_NAME'].indexOf('DATE') > -1) {
                        ...
                        if (value['COLUMN_HEADER'].indexOf('Miti') > -1) {
                            ...
                            if (value.DEFA_VALUE == null) {
                                ...
                                $("#nepaliDate5_Scrn2").val(nepalidate);
                            }
                            else {
    ...
    $scope.ConvertEngToNep = function () {
        ...
        $("#nepaliDate5_Scrn2" + splitteddate).val(nepalidate);
    ...
    $scope.ConvertNepToEng = function ($event) {
        var date = BS2AD($("#nepaliDate5_Scrn2").val());
        $("#englishdatedocument_scrn2").val($filter('date')(date, "dd-MMM-yyyy"));
        $('#nepaliDate5_Scrn2').trigger('change')
    };

    $scope.ConvertEngToNepang = function (data) {
        $("#nepaliDate5_Scrn2").val(AD2BS(data));
    };
    ...
    $scope.TodayDate = AD2BS(moment($("#englishdatedocument_scrn2").val()).format('YYYY-MM-DD'));
    ...
    $scope.print = function (event) {
        ...
        $scope.units = [];
        $scope.totalQty = 0;
        myInventoryScreen2Dropzone.processQueue();
    ...
    $scope.SaveDocumentFormData = function (param) {
        ...
        if ($scope.isCheckSaveValidationState) {
            $scope.$parent.isSecondScreenValidToSave = true;
            $scope.$emit('fromChildToParentValidateState', { state: true });
            return;
        }
        ...
        return response.then(function (data) {
            ...
            if (data.data.MESSAGE == "INSERTED") {
                ...
                if ($scope.ModuleCode == "03") {
                    ...
                    if ($scope.$parent.isSaveOnlyFlage == true) {
                        $scope.showSimplePopupAndReload("Voucher Saved Successfully! </br> Issued Voucher No: " + $scope.parentVoucherNo + " </br> Received Voucher No:" + data.data.VoucherNo);
                        return;
                    }

                    $scope.showSimplePopup("Voucher Saved Successfully! </br> Issued Voucher No: " + $scope.parentVoucherNo + " </br> Received Voucher No:" + data.data.VoucherNo);
                    // call for next step and loading management 
                    $scope.AfterSaveAndUpdateCalled();
                    return;
                }
                ...
                if ($scope.ModuleCode == "03") {
                    //   DisplayBarNotificationMessage("Voucher Saved Successfully! </br> Voucher no: " + data.data.VoucherNo);
                    $scope.showSimplePopup("Voucher Saved Successfully!");
    ...
    $scope.deleteIssueRecored = function () {
        //$scope.parentVoucherNo
        var deleteIssueRecord = "/api/InventoryApi/DeleteIssueVoucher?voucherno=" + $scope.parentVoucherNo;
        $http.delete(deleteIssueRecord)
            .then(function (res) {
                console.log(res);
                $route.reload();
            })
    ...
    $scope.AfterSaveAndUpdateCalled = function () {
        // Code to load next step using change route
        setTimeout(function () {
            var formUrl = "/api/SetupApi/GetNextScreenFormCode?formcode=" + $scope.formcode;
            $http.get(formUrl).then(function (response) {
                if (response && response.data) {
                    ...
                    } else if (response.data.Form_Code) {

                        $location.search('formcode', response.data.Form_Code);
                    } else {
                        $('#saveDocumentFormDataMainId').removeClass('active');
                        var landingUrl = window.location.protocol + "//" + window.location.host + "/DocumentTemplate/Home/Index#!DT/Inventory?formcode=" + 128;
                        //$window.location.href = landingUrl;
                        setTimeout(function () {
                            $window.location.href = landingUrl;
                            location.reload();
    ...
    $scope.SuccessPopupClick = function () {
        if ($scope.$parent.isSaveOnlyFlage == ture) {
            location.reload();
        } else {
            alert('called remove only');
    ...
    $scope.showSimplePopupAndReload = function (message) {

        //<button onclick="myPopupOkClick()" style="
        //            margin-top: 15px; padding: 6px 16px; background: #4CAF50; color: white;
        //            border: none; border-radius: 4px; cursor: pointer;">OK</button>
    ...
    $scope.getVoucherDetailForReferenceProduct = function () {
        ...
        if ($scope.masterModels.hasOwnProperty("PRODUCTION_QTY")) {
            ...
            if (!$scope.receivedFromParentData.Production_Qty) {
                if (!productionQty)
                    return;
            }
        ...
        // this mapping : data from parent
        if ($scope.receivedFromParentData && Object.keys($scope.receivedFromParentData).length > 0) {
            console.log("Object is not empty!");
            model.RoutingName = $scope.receivedFromParentData.RoutingName;
            model.Production_Qty = $scope.receivedFromParentData.Production_Qty;
        }
                            ...
                            if (cval.hasOwnProperty("ITEM_CODE")) {
                                ...
                                $http.get(ireq).then(function (results) {
                                    setTimeout(function () {
                                        ...
                                        $("#productsScrn2_" + ckey).data('kendoComboBox').dataSource.data([{ ItemCode: cval.ITEM_CODE, ItemDescription: results.data, Type: "code" }]);
                                    }, 0);
                                });
                            }
    ...
    $scope.openReferenceNoPopup = function (searchText) {
        ...
        setTimeout(function () {
            $('#searchModalReferanceNoViewScrn2Popup').find('.modal-dialog').addClass('large1-popup');
            var $dialog = $('#searchModalReferanceNoViewScrn2Popup').find('.modal-dialog');
            $dialog.css({
                'max-width': '80%',
                'width': '80%'
            });
            $('#searchModalReferanceNoViewScrn2Popup').modal('show'); // Show Bootstrap modal
        }, 100);
    ...
    $scope.$on('parentLTRNoKeyupEvent', function (event, dataLTRValue) {
        // do something with the data
        console.log('Received from parent:', dataLTRValue);
        $scope.masterModels['BATCH_NO'] = dataLTRValue;
    });

    $scope.$on('parentManulNoKeyupEvent', function (event, dataManualNoValue) {
        $scope.masterModels['MANUAL_NO'] = dataManualNoValue;
    });
