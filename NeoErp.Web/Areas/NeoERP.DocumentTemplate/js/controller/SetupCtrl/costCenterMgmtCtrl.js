DTModule.controller('costCenterMgmtCtrl', function ($scope, subLedgerMgmtService, $window, $filter) {
    console.log("controller hit");
    $scope.FormName = "Cost Center Management";
    $scope.selectedLedgerType = "08";
    ...
    $("#subLedgerAccountTree").kendoTreeView({
        ...
        // height: 400,
        select: onSubLedgerSelectForCostCenter,
        //scrollable: {
        //    virtual: true
        //}
    });
    ...
    function onSubLedgerSelectForCostCenter(e) {
        ...
        subLedgerMgmtGrid.dataSource.transport.options.read.url = window.location.protocol + "//" + window.location.host + "/api/SubLedgerMgmtApi/GetCostCenterMappingForGrid?subCode=" + $scope.selectedSubLedger.ACC_CODE + "&masterCode=" + $scope.selectedSubLedger.MASTER_ACC_CODE + "&preCode=" + $scope.selectedSubLedger.PRE_ACC_CODE;
    ...
    $scope.subLedgerMgmgGridOptions = {
        dataSource: {
            type: "json",
            transport: {
                read: "/api/SubLedgerMgmtApi/GetCostCenterMappingForGrid",
    ...
    $scope.applyLedgerMapping = function () {
        ...
        for (var i = 0; i < dataItem.length; i++) {
            ...
            if ($scope.selectedLedgerType === "08") {
                $scope.selectedItemList.push({
        ...
        if ($scope.selectedItemList.length > 0) {
            ...
            if ($scope.selectedLedgerType === "08") {
                var saveResult = subLedgerMgmtService.SaveCostCenterMapping(saveModel);
            }
            else {
    ...
    var getCustomerForLedgerUrl = window.location.protocol + "//" + window.location.host + "/api/SubLedgerMgmtApi/GetCostCenterSubLedger";
...
DTModule.service('subLedgerMgmtService', function ($http) {
    ...
    this.SaveCostCenterMapping = function (dataToSave) {
        var saveRes = $http({
            method: "POST",
            url: "/api/SubLedgerMgmtApi/SaveCostCenterMapping",
            data: JSON.stringify(dataToSave),
            dataType: "JSON"
        });
        return saveRes;
