...
namespace NeoERP.DocumentTemplate.Controllers
{
    public class TemplateController : Controller
    {
        ...
        public ActionResult Formtemplate(string formCode)
        {
            ...
            ViewBag.tableName = this._formtemplaterepo.GetTableNameByFormCode(formCode);
            return PartialView();
        }
        ...
        public PartialViewResult FolderTemplate(WebDesktopManagement webDesktopManagement)
        {
            ...
            foreach (var icon in Sales_Record)
            {
                ...
                SalesData.HREF = "/DocumentTemplate/Home/Index#!DT/formtemplate/" + icon.FORM_CODE;
            ...
            foreach (var icon in Finance_Record)
            {
                ...
                FianaceData.HREF = "/DocumentTemplate/Home/Index#!DT/FinanceVoucher?formcode=" + icon.FORM_CODE;
            ...
            foreach (var icon in Inventory_Record)
            {
                ...
                InventoryData.HREF = "/DocumentTemplate/Home/Index#!DT/Inventory?formcode=" + icon.FORM_CODE;
            ...
            var UpdateRecord = this._formtemplaterepo.GetAllUpdateItems();
            WebDesktopManagement updateData = new WebDesktopManagement();
            foreach (var icon in UpdateRecord)
            {
                Data.MENU_NO = icon.MENU_NO;
                Data.MENU_DESC = icon.MENU_DESC;
                Data.MENU_EDESC = icon.MENU_EDESC;
                ...
                Data.COLOR = icon.COLOR;
                Data.ICON_PATH = icon.ICON_PATH;
                Data.PREV_FOLDER_NAME = "Update";
                Data.NEW_FOLDER_NAME = "Update";
                Data.UNIQUE_ID = UniqueId();
                //exist or not
                bool ExistFolder = this._formtemplaterepo.FolderExistsOrNot("Update");
                if (!ExistFolder)
                {
                    this._formtemplaterepo.AddNewFolder("Update", "a7a1a1", "icon-home fontgreen");
                }


                if (!string.IsNullOrEmpty(updateData.MENU_NO))
                {
                    bool Exist = this._formtemplaterepo.MenuNoExistsOrNot(updateData.MENU_NO);
                    if (!Exist)
                    {
                        this._formtemplaterepo.AddWebDesktopManagement(updateData);
            ...
            foreach (var icon in productionRecords)
            {
                ...
                productionData.HREF = "/DocumentTemplate/Home/Index#!DT/Inventory?formcode=" + icon.FORM_CODE;
            ...
            return PartialView("~/Areas/NeoErp.DocumentTemplate/Views/Template/_FolderTemplate.cshtml");
        }
        [HttpPost]
