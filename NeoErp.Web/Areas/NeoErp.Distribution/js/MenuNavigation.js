...
function menuBreadCumList(data) {
    ...
    setTimeout(function () {
        var absulateUrlText = (location.pathname + location.hash.split('?')[0]);
        ...
        if (selectedURL == "Favourite") {
            ...
            for (k = 0; k < parentLength; k++) {
                if (data[k].MENU_EDESC == selectedURL) {
                    ...
                    var childLocation = location.hash.substr(14).split('?')[0];
