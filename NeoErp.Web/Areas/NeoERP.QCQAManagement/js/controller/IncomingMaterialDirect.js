QCQAModule.controller('IncomingMaterialDirectList', function ($scope, $rootScope, $http, $filter, $timeout, $routeParams) {
    $scope.voucherno = "";
    ...
    $("#ddlVendor").kendoDropDownList({
        ...
        dataTextField: "supplier_edesc",   // <- replace with actual property
        dataValueField: "SUPPLIER_CODE",    // <- replace with actual property
        dataSource: {
            transport: {
                read: {
                    url: window.location.protocol + "//" + window.location.host + "/api/IncomingMaterialDirectAPI/GetSupplierLists",
                    dataType: "json"
                }
            }
        },
    ...
    $scope.saveQCParameter = function () {
        ...
        var qcDetailList = {
            ...
            VOUCHER_NO: $scope.QC_NO,
            TRANSACTION_NO: $scope.voucherno,
            MANUAL_NO: $scope.MANUAL_NO,
            QUANTITY: $('#txtReceivedQty').val(),
            REMARKS: $('#txtRemarks').val(),
            VENDOR_NAME: $('#ddlVendor').val(),
            INVOICE_NO: $('#txtInvoiceNo').val(),
            VENDOR_NAME: $('#ddlVendor').val(),
            /*Thickness0: $scope.thick1 ? $scope.thick1 : undefined,*/
        ...
        $http.post('/api/IncomingMaterialDirectAPI/saveInComingMaterialSample', qcDetailList)
            .then(function (response) {
    ...
    if ($scope.voucherno != "undefined") {
        $http.get('/api/IncomingMaterialDirectAPI/GetEditMaterialDetailsSample?transactionno=' + $scope.voucherno)
            .then(function (response) {
                ...
                $scope.VENDOR_NAME = response.data.SUPPLIER_CODE;
                var dd2 = $("#ddlVendor").data("kendoDropDownList");

                dd2.value($scope.VENDOR_NAME);   // <-- now value exists
                $scope.to_location_code = response.data.to_location_code;
                $scope.invoice_no = response.data.invoice_no;
                $scope.GRN_NO = response.data.GRN_NO;
                $scope.RECEIPT_DATE = kendo.toString(kendo.parseDate(response.data.RECEIPT_DATE), 'M/d/yyyy')
                $scope.quantity = response.data.quantity;
                $scope.item_edesc = response.data.item_edesc;
                $scope.Remarks = response.data.REMARKS;
                $('.thick1').text(response.data.Thickness0);
                //$('.thick2').val(response.data.RollDiameter0);
                $scope.thick2 = response.data.RollDiameter0;
                $('.thick3').text(response.data.PH0);
                $('.thick8').text(response.data.Width0);
                $('.thick9').text(response.data.GSM0);
                $('.thick10').text(response.data.TensileCD0);
                $('.thick11').text(response.data.TensileMD0);
