planningModule.controller('EditTarget', function ($scope, $http, $route, $routeParams, $timeout) {
    ...
    $scope.init = async function () {
        try {
            console.log("Init", $scope.targetId);
            const targetId = $scope.targetId;
            const response = await $http.get(`/api/DistributionPlaningApi/GetTargetById?targetId=${targetId}`);
            const targetData = response.data;

            // 2️⃣ Bind response to $scope
            $scope.targetName = targetData.TARGET_NAME || '';
            $scope.targetType = targetData.TARGET_TYPE || null;
            $scope.Type = targetData.TARGET_SETUP_TYPE || null;
            $scope.customQty = targetData.TARGET_QUANTITY || 0;
            $scope.customAmt = targetData.TARGET_AMOUNT || 0;
            $scope.selectedSubTarget = $scope.subTargetTypeOptions.dataSource.find(
                st => st.value.trim() === (targetData.SUB_TARGET_TYPE || '').trim()
            ) || null;
            $scope.dateFilter = targetData.DATE_FILTER || null;
            $scope.bsFromDate = targetData.FROM_DATE ? new Date(targetData.FROM_DATE) : '';
            $scope.bsToDate = targetData.END_DATE ? new Date(targetData.END_DATE) : '';

            // Bind groups and selections
            $scope.selectedEmpGroup = targetData.EMP_GROUP || [];
            try {
                // ASSIGN_EMPLOYEE comes as stringified JSON, parse it
                $scope.selectedEmployees = targetData.ASSIGN_EMPLOYEE.map(empStr => {
                    try { return JSON.parse(empStr); } catch { return null; }
                }).filter(e => e !== null);
            } catch (e) {
                $scope.selectedEmployees = [];
            }
            $scope.selectedCusGroup = targetData.CUSTOMER_GROUP || [];
            $scope.selectedItmGroup = targetData.ITEM_GROUP || [];
            $scope.selectedIndividual = targetData.ITEMS || [];

            // 3️⃣ Refresh any dependent dropdowns
            if ($scope.employeeGroupOptions) $scope.employeeGroupOptions.dataSource.read();
            if ($scope.customerGroupOptions) $scope.customerGroupOptions.dataSource.read();
            if ($scope.itemGroupOptions) $scope.itemGroupOptions.dataSource.read();

            // 4️⃣ If needed, trigger sub-target change logic
            if ($scope.selectedSubTarget) $scope.onSubTargetChange();

            console.log('Target data loaded:', targetData);
            setTimeout(() => {
                if ($scope.employeeOptions) {
                    $scope.employeeOptions.dataSource.read();
                }
                if ($scope.itemGroupOptions) {
                    $scope.itemGroupOptions.dataSource.read();
                }
                if ($scope.individualOptions) {
                    $scope.individualOptions.dataSource.read();
                }
                if ($scope.customerGroupOptions) {
                    $scope.customerGroupOptions.dataSource.read();
                }
            }, 100);

        } catch (err) {
            console.error('Error loading target by ID', err);
            displayPopupNotification('Failed to load target data', 'error');
        }
    };
    $scope.init();
    ...
    $scope.employeeOptions = {
        ...
        dataTextField: $scope.Type === "SNG" ? "EMPLOYEE_EDESC" : "USER_NAME",
        ...
        dataSource: new kendo.data.DataSource({
            ...
            transport: {
                ...
                parameterMap: function () {
                    return ($scope.selectedEmpGroup || []).length
                        ? { filter: $scope.selectedEmpGroup.map(g => g.GROUPID).join(',') }
                        : {};
                }
            }
        }),
