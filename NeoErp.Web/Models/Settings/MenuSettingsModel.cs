...
namespace NeoErp.Models.Settings
{
    public class MenuSettingsModel : IMenuSettings
    {
        ...
        public MenuSettingsModel(ICacheManager cacheManager, IDbContext dbContext, IWorkContext workContext )
        {
        ...
        public List<MenuItemModel> GetMenuList()
        {
            ...
            string query = $@"SELECT DISTINCT
                                  CASE 
                                  WHEN LENGTH(M.VIRTUAL_PATH) < LENGTH(M.FULL_PATH) 
                                       AND M.FULL_PATH LIKE '%#!%'
                                      THEN M.FULL_PATH
                                      WHEN M.MODULE_CODE = '01' 
                                      THEN '/Sales/SalesHome/DashBoard' || M.VIRTUAL_PATH
                                      ELSE M.VIRTUAL_PATH
                                  END AS VIRTUAL_PATH,
                                      M.MENU_NO,
                                      M.MENU_EDESC,
                                      M.MODULE_CODE
                                  FROM WEB_MENU_MANAGEMENT M
                                  WHERE M.COMPANY_CODE = '{companyCode}'
                                    AND M.GROUP_SKU_FLAG = 'I'
                                    AND M.MENU_NO IN (
                                        SELECT C.MENU_NO 
                                        FROM WEB_MENU_CONTROL C 
                                  ...
                                  SELECT DISTINCT 
                                      '/DocumentTemplate/Home/Index#!DT/formtemplate/' || FS.FORM_CODE AS VIRTUAL_PATH,
                                      '0' AS MENU_NO,
                                      FS.FORM_EDESC AS MENU_EDESC,
                                      FS.MODULE_CODE
                                  FROM FORM_SETUP FS
                                  WHERE FS.COMPANY_CODE = '{companyCode}'
                                  AND FS.FORM_CODE IN (SELECT FORM_CODE FROM SC_FORM_CONTROL WHERE USER_NO = '{userNo}')";
            var menuList = _dbContext.SqlQuery<MenuItemModel>(query).ToList();
          return menuList;
        }
    }
}