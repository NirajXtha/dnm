using NeoErp;
using NeoErp.Core;
using NeoErp.Core.ComponentModel;
using NeoErp.Core.Models;
using NeoErp.Core.Plugins;
//using NeoErp.Core.Plugins.PluginEntities;
using System;
...
//[assembly: PreApplicationStartMethod(typeof(PluginAreaBootstrapper), "BreakCache")]
...
namespace NeoErp
{
    public class PluginAreaBootstrapper
    {
        ...
        private const string InstalledPluginsFilePath = "~/App_Data/InstalledModules.txt";
        private const string PluginsPath = "~/Areas";
        private const string ShadowCopyPath = "~/Areas/bin";// Bikalp change
      //  private const string ShadowCopyPath = "~/bin";
        ...
        public static void Init()
        {
            using (new WriteLockDisposable(Locker))
            {
                ...
                try
                {
                   ...
                   // var installedPluginSystemNames = PluginFileParser.ParseInstalledPluginsDataBase();
                    ...
                    foreach (var dfd in GetDescriptionFilesAndDescriptors(pluginFolder))
                    {
                        ...
                        if (!pluginDescriptor.SupportedVersions.Contains(NeoVersion.CurrentVersion, StringComparer.InvariantCultureIgnoreCase))
                        {
            ...
            //var currentAssemblies = AppDomain.CurrentDomain.GetAssemblies().Select(
            //    pluginAssembly => pluginAssembly.GetName().Name)
            //    .ToList();
            ...
            //    var addFile = Assembly.LoadFile(file);
            //    var name = addFile.GetName().Name;
            //    var plugins = PluginNames();
            //    if (!plugins.Contains(name) && !currentAssemblies.Contains(name))               
            //        PluginAssemblies.Add(addFile);
            //}
        ...
        public static void BreakCache()
        {
            ...
            var fullpath = tempFolder + @"\Areas\" + filename;

            // create our builders
            var asmBuilder = Thread.GetDomain()
                  .DefineDynamicAssembly(assemblyName, AssemblyBuilderAccess.RunAndSave, tempFolder + @"\Areas\");
        ...
        public static void MarkPluginAsInstalled(PluginDescriptor pluginDescription)
        {
            var systemName = pluginDescription.SystemName;
            ...
            if (!alreadyMarkedAsInstalled)
            {
                ...
                try
                {
                    var a = PluginFileParser.InsertModuleToDB(pluginDescription);
        ...
        /// </summary>
        /// <param name="systemName">Plugin system name</param>
        public static void MarkPluginAsUninstalled(PluginDescriptor pluginDescription)
        {
            var systemName = pluginDescription.SystemName;
            if (String.IsNullOrEmpty(systemName))
                throw new ArgumentNullException("systemName");
            ...
            if (alreadyMarkedAsInstalled)
            {
                ...
                try
                {
                    var a = PluginFileParser.UninstallModuleFormDB(pluginDescription);
                    installedPluginSystemNames.Remove(systemName);
                    //if (a > 0)
                    //{
                    //    installedPluginSystemNames.Remove(systemName);
        ...
        /// Mark plugin as installed
        /// </summary>
        /// <param name="systemName">Plugin Description</param>
        //public static void MarkPluginAsInstalled(PluginDescriptor pluginDescriptior)
        //{
        //    if (pluginDescriptior == null)
        //        throw new ArgumentNullException("pluginDescriptior");
        //   // NeoErp.Core.Models.NeoCoreEntity _db = new NeoErp.Core.Models.NeoCoreEntity();
        //    //var module = _db.Modules.Where(wh => wh.ModuleID == pluginDescriptior.SystemName).FirstOrDefault();
        //    //if (module == null)
        //    //{
        //    //    var NeoErpData = new Modules();
        //    //    NeoErpData.ModuleName = pluginDescriptior.FriendlyName;
        //    //    NeoErpData.ModuleID = pluginDescriptior.SystemName;
        //    //    NeoErpData.GroupID = pluginDescriptior.Group;
        //    //    NeoErpData.DisplayOrder = pluginDescriptior.DisplayOrder;
        //    //    NeoErpData.AllowDelete = false;
        //    //    NeoErpData.CreatedOn = DateTime.UtcNow;
        //    //    NeoErpData.DisplayOrder = pluginDescriptior.DisplayOrder;

        //    //    _db.Modules.Add(NeoErpData);
        //    //    _db.SaveChanges();
        //    //}
        ...
        //      throw new ArgumentNullException("systemName");
        // //   NeoErp.Core.Models.NeoCoreEntity _db = new  NeoErp.Core.Models.NeoCoreEntity();
        //    //var module = _db.Modules.Where(wh => wh.ModuleID == systemName).FirstOrDefault();
        ...
        /// Mark plugin as uninstalled
        /// </summary>
        public static void MarkPluginAsUninstalled()
        {
            var filePath = HostingEnvironment.MapPath(InstalledPluginsFilePath);
            if (File.Exists(filePath))
        ...
        private static IEnumerable<KeyValuePair<FileInfo, PluginDescriptor>> GetDescriptionFilesAndDescriptors(DirectoryInfo pluginFolder)
        {
            ...
            //it's required: http://www.NeoErpcommerce.com/boards/t/17455/load-plugins-based-on-their-displayorder-on-startup.aspx
        ...
        private static Assembly PerformFileDeploy(FileInfo plug)
        {
             if (plug.Directory.Parent == null)
                throw new InvalidOperationException("The plugin directory for the " + plug.Name +
                                                    " file exists in a folder outside of the allowed NeoErpCommerce folder heirarchy");
        ...
        private static FileInfo InitializeMediumTrust(FileInfo plug, DirectoryInfo shadowCopyPlugFolder)
        {
            ...
            if (shadowCopiedPlug.Exists)
            {
                ...
                else
                {
                    ...
                    //More info: http://www.NeoErpcommerce.com/boards/t/11511/access-error-NeoErpplugindiscountrulesbillingcountrydll.aspx?p=4#60838
        ...
        private static bool IsPackagePluginFolder(DirectoryInfo folder)
        {
            ...
            if (!folder.Parent.Name.Equals("Areas", StringComparison.InvariantCultureIgnoreCase) && !parent.Parent.Name.Equals("Areas", StringComparison.InvariantCultureIgnoreCase)) return false;
            return true;
        }
