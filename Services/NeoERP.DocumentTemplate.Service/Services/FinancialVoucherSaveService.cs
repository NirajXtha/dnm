...
namespace NeoERP.DocumentTemplate.Service.Services
{
    public class FinancialVoucherSaveService : IFinancialVoucherSaveService
    {
        ...
        public FinancialVoucherSaveService(IWorkContext workContext)
        {
           ...
           // _objectEntity = NeoErpCoreEntity.CreateForSelectedDatabase();
        }
        ...
        public bool SaveMasterColumnValue(FinancialVoucherDetail masterColumnValue, CommonFieldsForFinanacialVoucher commonValue,NeoErpCoreEntity dbContext = null)
        {
            try
            {
                ...
                REFERENCE_NO,SESSION_ROWID) VALUES('{commonValue.NewVoucherNumber}','{commonValue.DrTotal}',
                ...
                {commonValue.ExchangeRate},'{masterColumnValue.MANUAL_NO}','{masterColumnValue.SESSION_ROWID}')"; }

                else if (commonValue.TableName == "FA_SINGLE_VOUCHER")
                { insertmasterQuery = $@"INSERT INTO MASTER_TRANSACTION(VOUCHER_NO,VOUCHER_AMOUNT,FORM_CODE,
                COMPANY_CODE,BRANCH_CODE,CREATED_BY,DELETED_FLAG,CURRENCY_CODE,CREATED_DATE,VOUCHER_DATE,EXCHANGE_RATE,
                REFERENCE_NO,IS_SYNC_WITH_IRD,IS_REAL_TIME,SESSION_ROWID) VALUES('{commonValue.NewVoucherNumber}','{commonValue.DrTotal}',
                '{commonValue.FormCode}','{_workContext.CurrentUserinformation.company_code}','{_workContext.CurrentUserinformation.branch_code}',
                '{_workContext.CurrentUserinformation.login_code}','N','{commonValue.CurrencyFormat}',SYSDATE,
                TO_DATE('{masterColumnValue.VOUCHER_DATE}','DD-MON-YYYY'),
                {commonValue.ExchangeRate},'{masterColumnValue.MANUAL_NO}','N','N','{masterColumnValue.SESSION_ROWID}')"; }
                else
                { insertmasterQuery = $@"INSERT INTO MASTER_TRANSACTION(VOUCHER_NO,VOUCHER_AMOUNT,FORM_CODE,
                COMPANY_CODE,BRANCH_CODE,CREATED_BY,DELETED_FLAG,CURRENCY_CODE,CREATED_DATE,VOUCHER_DATE,EXCHANGE_RATE,
                REFERENCE_NO,SESSION_ROWID) VALUES('{commonValue.NewVoucherNumber}','{commonValue.DrTotal}',
        ...
        public bool SaveBudgetTransactionColumnValue(List<FinancialBudgetTransaction> budgetTransaction, CommonFieldsForFinanacialVoucher commonValue, NeoErpCoreEntity dbContext = null)
        {
                        ...
                        foreach (var bud in btrans.BUDGET)
                        {
                            if (bud.AMOUNT != null)
                            {
                                ...
                                string insertbudgettransQuery = $@"INSERT INTO BUDGET_TRANSACTION(
                                                                             ...
                                                                             {bud.AMOUNT},'{bud.NARRATION}','{_workContext.CurrentUserinformation.company_code}','{_workContext.CurrentUserinformation.branch_code}','{_workContext.CurrentUserinformation.login_code.ToUpper()}',SYSDATE,'N','NRS',{1},'Y','{btrans.ACC_CODE}','{btrans.SESSION_ROWID}')";
                                int rowsAffected = dbContext.ExecuteSqlCommand(insertbudgettransQuery);
                                if (rowsAffected > 0)
                                {
        ...
        public bool SaveFinancialTDSColumnValue(List<FinancialTDS> financialTDS, CommonFieldsForFinanacialVoucher commonValue, NeoErpCoreEntity dbContext = null)
        {
            bool dataSaved = false;
            try
            {
                        ...
                        foreach (var ctds in ftds.CHILDTDS)
                        {
                            if (ctds.TDS_AMOUNT > 0)
                            {
                                string inserttdsQuery = $@"INSERT INTO FA_DC_TDS_INVOICE(
                                                                              SERIAL_NO,INVOICE_NO,INVOICE_DATE,MANUAL_NO,FORM_CODE,CS_CODE,ACC_CODE,TDS_TYPE_CODE,TAXABLE_AMOUNT,TDS_PERCENT,TDS_AMOUNT,CURRENCY_CODE,EXCHANGE_RATE,COMPANY_CODE,BRANCH_CODE,DELETED_FLAG,CREATED_BY,CREATED_DATE,SESSION_ROWID,TRAN_TYPE,VOUCHER_NO,SUBMISSION_NO,MEETING_CODE,REMARKS)
                                                                              VALUES({ctds.SERIAL_NO},'{commonValue.NewVoucherNumber}',trunc(SYSDATE),'{commonValue.NewVoucherNumber}',{commonValue.FormCode},'{ctds.SUPPLIER_CODE}',
                                                                             '{ctds.ACC_CODE}','{ctds.TDS_TYPE_CODE}','{ctds.NET_AMOUNT}','{ctds.TDS_PERCENTAGE}',{ctds.TDS_AMOUNT},'NRS',{1},'{_workContext.CurrentUserinformation.company_code}','{_workContext.CurrentUserinformation.branch_code}','N','{_workContext.CurrentUserinformation.login_code.ToUpper()}',SYSDATE,'{ftds.SESSION_ROWID}','DR','{ctds.VOUCHER_NO}','{ctds.SUBMISSION_NO}','{ctds.MEETING_CODE}','{ftds.REMARKS}')";
                                int rowsAffected = dbContext.ExecuteSqlCommand(inserttdsQuery);
                                if (rowsAffected > 0)
                                {
                                    tdsSerial++;
                                    dataSaved = true;
        ...
        public bool SaveFinancialVATValue(List<FinancialVAT> financialVAT, CommonFieldsForFinanacialVoucher commonValue, NeoErpCoreEntity dbContext = null)
        {
            bool dataSaved = false;
            try
            {
                ...
                foreach (var fvat in financialVAT)
                {
                    ...
                    {
                        var vehiclePurFlag = fvat.VEHICLE_PUR_FLAG == "True" ? "Y" : "N";
                        var exemptedFlag = fvat.EXEMPTED_FLAG == "True" ? "Y" : "N";
                        ...
                        if (fvat.CHILDVAT.Count > 0)
                        {
                            ...
                            {
                                var csCode = fvat.DOC_TYPE == "P" ? fv.SUPPLIER_CODE : fv.CUSTOMER_CODE;

                                    string insertvatQuery = $@"INSERT INTO FA_DC_VAT_INVOICE(
                                                                          INVOICE_NO,MANUAL_NO,INVOICE_DATE,FORM_CODE,CS_CODE,TAXABLE_AMOUNT,VAT_AMOUNT,P_TYPE,DOC_TYPE,REMARKS,CURRENCY_CODE,EXCHANGE_RATE,COMPANY_CODE,BRANCH_CODE,DELETED_FLAG,CREATED_BY,CREATED_DATE,SERIAL_NO,SESSION_ROWID,ACC_CODE,PURCHASE_ITEM,PURCHASE_QUANTITY,PURCHASE_UNIT,PP_NO,VEHICLE_PUR_FLAG,EXEMPTED_FLAG)
                                                                          VALUES('{commonValue.NewVoucherNumber}','{fvat.MANUAL_NO}', trunc(SYSDATE),'{commonValue.FormCode}','{csCode}','{fv.TAXABLE_AMOUNT}',
                                                                         '{fv.VAT_AMOUNT}','{fvat.TYPE}','{fvat.DOC_TYPE}','{fvat.REMARKS}','NRS',1,'{_workContext.CurrentUserinformation.company_code}','{_workContext.CurrentUserinformation.branch_code}','N','{_workContext.CurrentUserinformation.login_code.ToUpper()}',SYSDATE,{fv.SERIAL_NO},'{fvat.SESSION_ROWID}','{fvat.ACC_CODE}','{fvat.PURCHASE_ITEM}','{fvat.PURCHASE_QUANTITY}','{fvat.PURCHASE_UNIT}',
                                                                          '{fvat.PP_NO}','{vehiclePurFlag}','{exemptedFlag}')";
                                    int rowsAffected = dbContext.ExecuteSqlCommand(insertvatQuery);
                                    if (rowsAffected > 0)
                                    {
                                        vatSerial++;
                                        dataSaved = true;
        ...
        public bool SaveFinancialSubLedgerColumnValue(List<FinancialSubLedger> financialSubLedgers, CommonFieldsForFinanacialVoucher commonValue,NeoErpCoreEntity dbContext=null)
        {
            bool dataSaved = false;
            try
            {
                        ...
                        foreach (var sbLeg in fsl.SUBLEDGER)
                        {
                            if (sbLeg.SUB_CODE != "" && !string.IsNullOrEmpty(sbLeg.SUB_CODE))
                            {
                                ...
                                string insertSubLedgerQuery = $@"INSERT INTO FA_VOUCHER_SUB_DETAIL (TRANSACTION_NO,FORM_CODE,
                                                                   ...
                                                                   SERIAL_NO,CURRENCY_CODE,EXCHANGE_RATE,SYN_ROWID,PARTY_TYPE_CODE,SESSION_ROWID) 
                                                                  ...
                                                                  ,'{_workContext.CurrentUserinformation.company_code}','{_workContext.CurrentUserinformation.login_code.ToUpper()}',SYSDATE,'N','{sbLeg.SERIAL_NO}','{commonValue.CurrencyFormat}',{commonValue.ExchangeRate},'','{sbLeg.PARTY_TYPE_CODE}','{fsl.SESSION_ROWID}')";
                                int rowsAffected = dbContext.ExecuteSqlCommand(insertSubLedgerQuery);
                                if (rowsAffected > 0)
                                {
                                    subSerial++;
                                    dataSaved = true;
                ...
                if (chargetransaction.Count > 0)
                {
                    foreach (var chargetran in chargetransaction)
                    {
                        ...
                        else
                        {
                            ...
                            string queryCheck = $@"select charge_amount from CHARGE_TRANSACTION where REFERENCE_NO = '{chargetran.REFERENCE_NO}' and item_code = '{chargetran.ITEM_CODE}' and acc_code = '{chargetran.ACC_CODE}' and charge_code = '{chargetran.CHARGE_CODE}' and form_code = '{chargetran.FORM_CODE}' AND SERIAL_NO = '{chargetran.SERIAL_NO}' and created_by = '{_workContext.CurrentUserinformation.login_code.ToUpper()}' and company_code = '{_workContext.CurrentUserinformation.company_code}' and branch_code = '{_workContext.CurrentUserinformation.branch_code}' and DELETED_FLAG = 'N'";
