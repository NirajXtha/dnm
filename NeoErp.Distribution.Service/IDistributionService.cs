   ...
   public interface IDistributionService
    {
        ...
        dynamic GetVehicleSetup(User userinfo);
        dynamic UpdateVehicleSetup(string status, string id, User userinfo);
        dynamic GetLocationWiseStock(string orderNo, User userinfo);
        ...
        List<MrVisitedTrackingRecord> GetCurrentMrVistedRoute(string spCode, string companyCode = "01");
        ...
        dynamic GetClaimReport(ReportFiltersModel model, User userInfo);
        ...
        dynamic UpdateCollectionPdc(dynamic data, User userInfo);
        ...
        List<DeliveryModel> GetDeliveryReport(string Type, string status, ReportFiltersModel model, User userInfo);
        ...
        List<NotVisitedCustomerModel> GetNotVisitedCustomer(ReportFiltersModel filter, User userinfo, string Distributor = "", string Reseller = "");
        List<DailyActivityModel> GetDailyAcivityList(ReportFiltersModel model, User userInfo);
        dynamic GetAllUnitFilter(User user);
        dynamic GetAllDeliveryStatusFilter(User user);
        dynamic GetOutletClosingReport(ReportFiltersModel model, string unitFilter, User userInfo);
        dynamic GetResellerClosingStock(ReportFiltersModel model, string unitFilter, User userInfo);
        dynamic GetFarmerProblemRecommendation(ReportFiltersModel model, User userInfo);
        ...
        List<TargetModelPLan> GetTargetPlan(string subTargetType, string filterType, string dateFilter);
        ...
        FinalDataModel FinalData(string employeeCode, string companyCode);
        FinalDataModel ProcessCustomerData(User userInfo, List<CustomerData> customerDataList);

        void SaveData(User userInfo, List<CustomerData>  dataToSave);
        void AccessData(User userInfo, List<ManageAccessModal> dataToSave);

        List<ManageAccessModal> StoredAccessData(User userInfo);
    }
